...
import sys
sys.path.append('/projectnb/textconv/bkulis/DiffWave/metric_learning/pytorch-metric-learning/src')
...
#     #compute new loss 1
#     loss_func2 = NTXentLoss() 
#     #diffusion_steps = torch.mul(torch.ones(size=(B,1,1)), T).cuda()
#     diffusion_steps_subset = diffusion_steps[0,:,:].unsqueeze(0)
#     source_output_on_noise = net_source((z, diffusion_steps.view(B,1),), mel_spec=mel_spec)
#     first_noise_tensor = z[0,:,:].unsqueeze(0)
#     target_output_on_noise = net((first_noise_tensor, diffusion_steps_subset.view(1,1),), mel_spec=mel_spec)
#     embeddings = torch.cat((target_output_on_noise, source_output_on_noise))
#     embeddings = torch.reshape(embeddings, (B+1, C*L))
#     labels = torch.arange(B+1)
#     labels[1] = labels[0]
#     new_loss_1 = loss_func2(embeddings,labels)
#     return loss_fn(epsilon_theta, z) + 200*new_loss_1
...
def training_loss(net, loss_fn, audio, diffusion_hyperparams, mel_spec=None):
    ...
    loss_func_contrastive = NTXentLoss()
    ...
    negative_output = net((audio, diffusion_steps.view(B,1),), mel_spec=mel_spec)  # the negative pair uses different audio
    embeddings = torch.cat((positive_output, negative_output))
    embeddings = torch.reshape(embeddings, (2*B, C*L))
    labels = torch.arange(2*B) // 2  # create labels for contrastive loss: [0, 0, 1, 1, 2, 2, ..., B-1, B-1]
    contrastive_loss = loss_func_contrastive(embeddings,labels)

    return loss_fn(epsilon_theta, z) + 200*contrastive_loss
